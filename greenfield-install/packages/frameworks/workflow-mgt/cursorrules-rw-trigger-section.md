...
2. **Bump Version** - **MANDATORY STEP-BY-STEP PROCESS (DO NOT SKIP ANY STEP):**
   ...
   **D. VALIDATE BEFORE UPDATING:**
   ...
   - Verify: `VERSION_BUILD` matches resolver `next_build`
...
- **RW Maintenance (T103):** Manual attribution only—when releasing RW maintenance work (Step 7 fixes, validator updates, doc corrections), set version to E2:S16:T03 and increment BUILD. See Epic 2 Story 1 T06 (RW Maintenance Perpetual Task Pattern).
3. **Create Detailed Changelog** - Create detailed changelog in changelog archive directory. **Use config:** If `rw-config.yaml` exists, read `changelog_dir` from config. Otherwise, use `{changelog_archive_path}/CHANGELOG_v{version}.md` as fallback. Full timestamp (`YYYY-MM-DD HH:MM:SS UTC`). **CRITICAL:** Timestamp is IMMUTABLE once written - never edit the `**Release Date:**` field.
   - **BR-074 immutability guard:** Never overwrite or delete an existing `CHANGELOG_v{version}.md`; always create a new file. Use the post-bump version (just written to `version.py`) for the `Internal Version` header.
...
6. **Update BR/FR Docs** - Update Bug Reports and Feature Requests with fix attempt information. **Use config:** If `rw-config.yaml` exists, read `fbu_root` from config. Otherwise, use `docs/kanban/fbu` as fallback. **Purpose:** Document flaws, attempted fixes, and verification status so that if a bug isn't squashed, the next build can be informed by previous attempts.
   - **For Bug Reports (BR):**
...
9.7. **Actions CI Parity Gate (BLOCKING — E02:S16:T05 / FR-112 / BR-104)** — Run `validate_actions_ci_parity.py --strict` before Step 10 on full `RW`. Path-aware mirror of required GitHub Actions workflows; `--strict` fails when zero checks match unless `--allow-path-skip` (`RW -d` docs-only only). Non-zero exit → **RW ABORTED**. **CQG is IDW Phase 6b** (ADR-022 v0.0.2), not RW — green CodeQL ≠ green Tests. Before Step 12 (`--push`) or operator batch push: `--strict --all` then `validate_github_actions_remote.py --strict`.
10. **Commit Changes** - Build commit via `build_rw_commit_message.py` (or equivalent): **subject** `Release v{semver}: {summary}` (external SemVer — GitHub Actions run title on push); **body** `Internal: v{internal}\n\nEpic: {epic} | Story: {story} | Task: {task}`
...
12. **Push to Remote** — **NEVER by default (UXR-024).** Run **only** when user typed **`--push`** in the RW trigger.
    ...
    - **With `--push`:** Re-run Step 9.7 `--strict --all` and `validate_github_actions_remote.py --strict` immediately before push; then push epic branch and release-scoped tags (DO NOT push to main unless ready to deploy).
