(self["webpackChunkportal"] = self["webpackChunkportal"] || []).push([["51852"], {
58291(module) {
/**
...
95655(__unused_rspack_module, __unused_rspack___webpack_exports__, __webpack_require__) {
"use strict";
...
var generated_constants = __webpack_require__(86897);
 ...
 */ function tokenize(text, language) {
    ...
    if (language.length === 1 && [
        ...
        return (lunr_default())[language[0]].tokenizer(text).map((token)=>token.toString());
    }
 ...
 */ function smartQueries(tokens, zhDictionary) {
        ...
        return [
            {
                ...
                term: tokens.map((value)=>({
                        value,
                        presence: (lunr_default()).Query.presence.REQUIRED,
                        wildcard: (lunr_default()).Query.wildcard.LEADING | (lunr_default()).Query.wildcard.TRAILING
                    }))
            }
        ];
    }
    ...
    for (const lang of generated_constants/* .language */.BH){
        if (lang === "en") {
            if (!generated_constants/* .removeDefaultStopWordFilter.includes */.sx.includes(lang)) {
                stopWordPipelines.unshift((lunr_default()).stopWordFilter);
            }
        } else {
            const lunrLang = (lunr_default())[lang];
            if (lunrLang.stopWordFilter && !generated_constants/* .removeDefaultStopWordFilter.includes */.sx.includes(lang)) {
    ...
    const distance = Math.max(0, (/* inlined export .fuzzyMatchingDistance */1));
...
function getQueriesMaybeTyping(terms, editDistance) {
    return termsToQueries(terms, editDistance).concat(termsToQueries(// Ignore terms whose last token already has a trailing wildcard,
    ...
    return terms.flatMap((term)=>{
        const query = {
            ...
            term: term.map((item)=>{
                ...
                const trailing = maybeTyping ? item.trailing || item.maybeTyping : item.trailing;
                const distance = editDistance > 0 && item.value.length > editDistance ? editDistance : undefined;
                return {
                    value: item.value,
                    presence: (lunr_default()).Query.presence.REQUIRED,
                    wildcard: trailing ? (lunr_default()).Query.wildcard.TRAILING : (lunr_default()).Query.wildcard.NONE,
...
function sortSearchResults(results) {
    ...
    results.sort((a, b)=>{
        let aPageIndex = (a.type === interfaces_SearchDocumentType.Heading || a.type === interfaces_SearchDocumentType.Content || a.type === interfaces_SearchDocumentType.Description) && a.page ? results.findIndex((item)=>item.document === a.page) : a.index;
        let bPageIndex = (b.type === interfaces_SearchDocumentType.Heading || b.type === interfaces_SearchDocumentType.Content || b.type === interfaces_SearchDocumentType.Description) && b.page ? results.findIndex((item)=>item.document === b.page) : b.index;
        if (aPageIndex === -1) {
            aPageIndex = a.index;
        }
        if (bPageIndex === -1) {
...
function processTreeStatusOfSearchResults(results) {
    results.forEach((item, i)=>{
        if (i > 0 && item.page && results.slice(0, i).some((prev)=>(prev.type === interfaces_SearchDocumentType.Keywords ? prev.page : prev.document) === item.page)) {
...
class SearchWorker {
    ...
    async search(baseUrl, searchContext, input, limit) {
        const rawTokens = tokenize(input, generated_constants/* .language */.BH);
        ...
        search: for (const { term, tokens } of queries){
            for (const { documents, index, type } of wrappedIndexes){
                ...
                }).slice(0, limit)// Remove duplicated results.
                .filter((result)=>!results.some((item)=>item.document.i.toString() === result.ref)).slice(0, limit - results.length).map((result)=>{
                    const document = documents.find((doc)=>doc.i.toString() === result.ref);
                    return {
                        document,
                        type,
                        page: type !== interfaces_SearchDocumentType.Title && wrappedIndexes[0].documents.find((doc)=>doc.i === document.p),
...
async function legacyFetchIndexes(baseUrl, searchContext) {
    const url = `${baseUrl}${generated_constants/* .searchIndexUrl.replace */.IH.replace("{dir}", searchContext ? `-${searchContext.replace(/\//g, "-")}` : "")}`;
    ...
    const wrappedIndexes = json.map(({ documents, index }, type)=>({
            type: type,
            documents,
            index: lunr_default().Index.load(index)
        }));
