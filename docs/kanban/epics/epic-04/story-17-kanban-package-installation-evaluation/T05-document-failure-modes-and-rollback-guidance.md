...
4. Re-run validation: `python3 scripts/validate_installation.py --kanban-path docs/kanban`
...
2. **Restore from backup:**
   ...
   # Restore from backup
   rm -rf docs/kanban
   cp -r docs/project-management/_backup-YYYYMMDD-HHMMSS docs/kanban
   ```
...
1. **Manual backup before proceeding:**
   ...
   cp -r docs/kanban docs/kanban-backup-manual
   ```
2. **Fix backup issues:**
   - Free up disk space
...
1. **Create manual backup:**
   ```bash
   cp -r docs/kanban docs/kanban-backup-pre-install
   ```
