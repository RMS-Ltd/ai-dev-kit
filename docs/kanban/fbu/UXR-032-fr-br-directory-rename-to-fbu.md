...
**IPP:** [IPP-E04S19T14](../../implementation-cycles/IPP-E04S19T14-fr-br-directory-rename-to-fbu.md)
...
Does aligning the intake **directory** name with the **FBU** collective term reduce operator confusion, shorten paths in agent context, and complete the naming normalization started in E04:S19:T06 — without breaking adopters, installers, or the thousands of existing `fbu/` cross-links?
...
- Compared terminology in `intake-structure.md`, `intake-completed.md`, and `kboard.md` (already **FBU**-labelled) against the persistent `fbu/` path segment.
- Scanned framework defaults (`fbu_root`, `detect_fbu_root`, UKW workflow YAML) and install output contracts.
...
1. **Terminology drift:** Docs and boards say **FBU**; the directory and config key still say **`fr-br`** / **`fbu_root`** — a daily mismatch for operators and agents.
2. **Small surface, large ripple:** One directory rename (~254 markdown files) fans out to `rw-config.yaml`, RW/UKW/intake scripts, packaged `greenfield-install/`, validator defaults, skills, templates, and corpus-wide relative links (`../../fbu/…`).
3. **Install contract:** Fresh installs via `install_release_workflow.py` mode C **detect and emit** `fbu_root`; adopters and brownfield repos will need read-compat or migration guidance ([BR-083](BR-083-rw-install-default-patterns-mismatch-fresh-kanban-layout.md) precedent).
...
- Agent cold-start and intake skills reference `fbu/` while governance policy says **FBU**.
- New adopters see `fbu_root` in generated `rw-config.yaml` — inconsistent with shipped terminology.
...
- [ ] **R2:** Rename config key `fr_br_root` → `fbu_root` in schema, `rw-config.yaml`, and install detection (`detect_fbu_root` with `fbu_root` read-compat for one transition window).
- [ ] **R3:** Physical `git mv` of `docs/kanban/fbu/` → `docs/kanban/fbu/`; update all in-repo relative links (exclude immutable changelog archive unless policy allows).
...
- [ ] **R6:** Add validator or CI grep guard preventing reintroduction of `docs/kanban/fbu` in new writes (phased: warn → error).
...
| -------- | -------- | -------- |
| Intake directory | `docs/kanban/fbu/` | `docs/kanban/fbu/` |
| `rw-config.yaml` key | `fbu_root` | `fbu_root` (+ read-compat alias) |
| Install detection | `detect_fbu_root()` | `detect_fbu_root()` |
| UKW workflow YAML | `fbu_root: fr-br` | `fbu_root: fbu` |
| Intake guides / skills | `fbu/*.md` paths | `fbu/*.md` |
| `intake-structure.md` links | `fbu/FR-…` | `fbu/FR-…` |
| Greenfield mirror | same as packages | synced |
| Historical changelogs | frozen references | optional redirect note only |

**Estimated corpus touch (2026-06-23 scan):** 250+ files under `fbu/`; 100+ framework/config references to `fbu_root` / `fr-br` path strings across active (non-archive) trees.
...
- [intake-structure.md](../intake-structure.md) — already titled **FBU Structure** but links into `fbu/`
- `rw-config.yaml` (repo root) — `fbu_root: docs/kanban/fbu`
...
- [FR-118](FR-118-promote-kanban-to-docs-kanban-retire-project-management-shell.md) — mentions `fbu_root` in config phase (coordinate if both land)
...
- Task: T14 — `fbu/` directory rename to `fbu/` (UXR-032)
- Version: *(pending RW)*
...
- **Config compat:** Recommend read-tolerance for `fbu_root` in validators/installers for ≥1 release cycle (mirror UXR-017 phased migration).
