...
**Implementing Task:** [E06:S09:T45](../epics/epic-06/story-09-ai-dev-kit-installation-and-adopter-integration/T45-board-timestamp-carry-forward-fr144.md)
...
- [ ] **FR-144-F1:** UKW Step 6 — forbid bulk `Last modified:` = session date when legacy root or timestamp index exists; run restoration report first.
- [ ] **FR-144-F2:** KMA Step 4 — invoke timestamp restoration (or kit equivalent) before `kboard.md` rebuild.
- [ ] **FR-144-F3:** `discover_board_gaps.py` optional Part (c): flag rows where all dates are identical (homogeneity advisory).
- [ ] **FR-144-F4:** KMA ingest emits `timestamp-index.json` alongside `kma-ingest.json`.
- [ ] **FR-144-F5:** Step 4 validation gate: flat-date board fails or warns when legacy corpus available.

### Non-functional

- [ ] **FR-144-NF1:** Align with FR-130 / ADR-029 — substantive work only; no batch round-hour buckets.
- [ ] **FR-144-NF2:** Semantic lineage map contract for bootstrap `E:S:T` → legacy predecessor (adopter-supplied YAML).

---

## Acceptance criteria

- [ ] **AC1:** UKW on brownfield/guided path does not flatten all row stamps to session date when legacy index non-empty.
- [ ] **AC2:** KMA Step 4 guide mandates timestamp carry-forward checklist row.
- [ ] **AC3:** Validator or scorer flags boards with ≥3 identical `Last modified:` stamps post-UKW (extends FR-097 hygiene).
- [ ] **AC4:** SBL attempt 12 K-02 cross-linked; reference SBL tool pattern documented for kit port.
