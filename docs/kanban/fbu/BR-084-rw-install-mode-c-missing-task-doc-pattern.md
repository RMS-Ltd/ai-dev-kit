...
Missing: `task_doc_pattern`, `fbu_root`.
...
3. **`generate_rw_config_yaml`** — omits `task_doc_pattern` (and optionally `fbu_root`) when `use_kanban: true`.
...
- [x] Generated config includes `task_doc_pattern` (+ `fbu_root` when `fbu/` exists).
