...
| **C — Config & runtime** | `rw-config.yaml` (`kanban_root`, `fbu_root`); `.cursorrules`, `AGENTS.md`, `CLAUDE.md`, `project-agent-manifest.json`; pre-commit hooks |
...
- [ ] **FR-118-F5 (Phase C):** `rw-config.yaml` `kanban_root: docs/kanban` and `fbu_root: docs/kanban/fbu`; RW/UKW/intake validators pass on `dev`.
