...
- [ ] **FR-127-F1:** Agent ingests full legacy kanban corpus (epic docs, story files, task references, board state) — not directory listing only.
- [ ] **FR-127-F2:** Agent proposes target structure optimised for **project domain**, with explicit keep/drop/merge rationale.
- [ ] **FR-127-F3:** Operator review gate before file writes; refinements via dialogue.
- [ ] **FR-127-F4:** Agent executes migration (synthesised epic docs, renamed stories, cross-references) and runs sign-off validation.
- [ ] **FR-127-F5:** Deprecate or gate migration modes on the legacy tool pipeline; document `--mode fresh` as the retained automated path.

### Non-functional

- [ ] **FR-127-NF1:** Preserve legacy source tree (dual-tree / archive) per FR-081 brownfield contract.
...
- [ ] SBL-style legacy corpus (19 epics, 80+ stories, non-canonical names) migrates successfully via agent workflow.
- [ ] Tool pipeline migration modes documented as deprecated with pointer to agent workflow.
- [ ] INSTALL / brownfield guide updated with agentic migration profile.
