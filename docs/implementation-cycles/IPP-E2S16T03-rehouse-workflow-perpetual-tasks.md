...
| RF6 | Placement guardrail: reject perpetual tasks outside `E2:S16` unless `Perpetual Override Rationale:` present | IPP-E2S16T1 §2.6 |
| RF7 | Numbering guardrail: `T1xx` hard-fails without `Historical Anchor:`; active work uses `Txx` | IPP-E2S16T1 §2.6 |
| RF8 | Marker guardrail: warn when Story 016 perpetual lanes lack `Task Type: Perpetual Maintenance` | IPP-E2S16T1 §2.6 |
...
| RNF3 | Legacy artifact `IPW-E6S7T103` must be deprecated or redirected without breaking links | Archive hygiene |
...
- **In scope:**
  ...
  - Archive legacy `IPW-E6S7T103` draft with redirect notice.
...
**Outcome:** **EXEMPT** — governing docs: [dev-kit-versioning-policy.md](../architecture/standards-and-adrs/dev-kit-versioning-policy.md) §6.1.1; [IPP-E2S16T1](IPP-E2S16T01-canonical-perpetual-ongoing-tasks-story.md) §2.6 guardrails (E1–E5 pass).

### 1.4 Tranche 3 functional requirements (additive)

| ID | Requirement | Source |
| -- | ----------- | ------ |
| RF-T03-1 | Close BR-067 after R1–R5 regression evidence | BR-067 |
| RF-T03-2 | AC4 full satisfaction only after T02 reference map (3B) | T03 AC4; T02 |
| RF-T03-3 | Active-surface link hygiene (kanban, open FR/BR) | IPP rolling backlog |
| RF-T03-4 | Host task remains `IN PROGRESS` (perpetual) | IPP-E2S16T1 |
...
| T2 | `test_validate_version_bump_passes_for_perpetual_task` (T101) | Mock task file named `T101-*.md` (not `T03-*.md`) under epic-06/story-007 so `locate_task_doc` finds it. |
| T3 | `test_validate_version_bump_build_increment_perpetual` | Same as T2 — mock file must match the task number under test. |
| T4 | `test_validate_version_bump_passes_for_t103` (T103) | Mock task file named `T103-*.md` under epic-06/story-007 so `locate_task_doc` finds it. |
...
| T7 | Legacy draft redirect | All inbound links to `IPW-E6S7T103` resolve to T03 or deprecation notice. |

### 3.1 Tranche 3 test design

| ID | Behavior / layer | Expected check |
| -- | ---------------- | -------------- |
| T-T03-1 | Unit | `pytest test_validate_version_bump.py -x` → 0 exit |
| T-T03-2 | Integration | BR-067 R1–R5 matrix documented PASS in BR-067 + T03 |
| T-T03-3 | Integration | Perpetual RW: `validate_version_bump.py --strict --requested E02:S16:T03 --art` after Step 2 bump |
| T-T03-4 | Structural | Active kanban: no stale `epic-06/story-007/.../T03` for perpetual routing |
| T-T03-5 | Structural (3B) | T02-gated exhaustive migration — deferred until T02 AC1–AC5 |
...
| **5** | Fix `test_validate_version_bump_passes_for_t103`: rename/create mock task file to `T103-*.md` under epic-06/story-007. | Test passes |
...
| **8** | Archive legacy `IPW-E6S7T103`: add deprecation banner at top, update status to `Deprecated`, add redirect link to T03 task doc. | Draft no longer appears active |
...
#### Document C: Legacy IPW draft (`IPW-E6S7T103-release-workflow-maintenance-perpetual.md`)
...
#### Document D: story-016 doc (`story-16-perpetual-ongoing-workflow-operations.md`)
...
- [x] Legacy `IPW-E6S7T103` deprecated with T03 redirect.
...
**Entry gate: SATISFIED** — [`IPP-E2S16T2-perpetual-task-inventory.md`](IPP-E2S16T02-perpetual-task-inventory.md); T02 **COMPLETE** (`v0.2.16.2+2`).

**3B execution: COMPLETE (2026-06-03)** — ACTIVE_FIX migration (`apply_e6s07_path_migration_3b.py`); `docs/` `epic-06/story-007` **167→153** (101→94 files); kanban **35→18** (22→14 files). ARCHIVE_STUB + KB-INDEX deferred per IPP-E2S16T2 §2.6. T03 AC4 `[x]`; release **`RW E02:S16:T03 --art`**.
...
| IPP | [`IPP-E2S16T2-perpetual-task-inventory.md`](IPP-E2S16T02-perpetual-task-inventory.md) |
| Reference baseline | 167 `epic-06/story-007` occurrences / 101 files under `docs/` |
...
- [x] T03 task doc links corrected to canonical `IPP-E2S16T3` filename.
