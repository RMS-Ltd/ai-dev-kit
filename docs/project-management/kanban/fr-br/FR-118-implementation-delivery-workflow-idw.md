...
# FR-118: Implementation Delivery Workflow (IDW) with optional `--rw` chain
...
- [x] **FR-118-F1:** `.claude/commands/idw.md` — self-contained IDW command (implementation mode; not plan mode).
- [x] **FR-118-F2:** KB agent execution guide for IDW under workflow-mgt vwmp.
- [x] **FR-118-F3:** Workflow registry entry `implementation-delivery-workflow` with triggers `IDW`.
- [x] **FR-118-F4:** `CLAUDE.md`, `.cursorrules`, `AGENTS.md`, `project-agent-manifest.json` routing for `IDW` / `idw`.
- [x] **FR-118-F5:** Cheatsheet §4b + sequences; `--rw` flag documented (lowercase).
- [x] **FR-118-F6:** Invoking `IDW E:S:T` satisfies FR-083 explicit implementation authorization.
- [x] **FR-118-F7:** `IDW E:S:T --rw` chains RW after `IDW COMPLETE`; `--push` / `--art` forward to RW when present.

### Non-functional

- [x] **FR-118-NF1:** IDW does not commit, tag, or bump version (RW owns release).
- [x] **FR-118-NF2:** IPW does not chain to IDW or RW by default.
