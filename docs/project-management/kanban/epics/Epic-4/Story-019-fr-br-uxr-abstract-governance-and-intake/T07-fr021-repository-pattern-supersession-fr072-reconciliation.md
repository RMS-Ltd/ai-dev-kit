---
lifecycle: evergreen
ttl_days: null
created_at: 2026-05-29T17:00:00Z
expires_at: null
housekeeping_policy: keep
---

# Epic 4, Story 19, Task 7: FR-021 repository pattern supersession (FR-072 reconciliation)

**Task ID:** E04:S19:T07  
**Status:** TODO  
**Priority:** MEDIUM  
**Created:** 2026-05-29  
**Last updated:** 2026-05-29 (**RW -k** **v0.4.19.7+1** — FR-021 semantic task filed)  
**Version Anchor:** v0.4.19.7+1  
**Code:** E04S19T07

**Upstream:** [FR-021 — FR/BR/UXR repository stories](../../../fr-br/FR-021-fr-br-uxr-repository-stories.md)  
**Related:** [FR-072](../../../fr-br/FR-072-uniform-repository-abstract-space-kanban-rules.md), [E04:S19:T08](T08-fr072-approach-d-phase-d2-d4-migration-and-doc-sweep.md)  
**Historical registry:** [E05:S01:T21](../../Epic-5/Story-001-fr-repo/T21-fr-br-uxr-repository-stories.md)

Publication Status: NOT_APPLICABLE

---

## Input

- [FR-021](../../../fr-br/FR-021-fr-br-uxr-repository-stories.md); [FR-072](../../../fr-br/FR-072-uniform-repository-abstract-space-kanban-rules.md) (Approach D).

---

## Deliverable

Document and execute **Approach D** supersession of dedicated FR/BR/UXR **repository stories** as primary intake targets: semantic delivery stories own implementing tasks; S00/S01 repo stories remain historical traceability only.

## Acceptance criteria

- [ ] **AC1:** Intake guide + kanban governance state that new intake must not open primary tasks under E5:S01 / E6:S01 / E7:S00 repository stories (exceptions documented).
- [ ] **AC2:** **FR-021** resolution text references this task and **FR-072** decision memo.
- [ ] **AC3:** Story **E4:S12** / **E4:S19** checklists cross-link; no contradictory “file under FR repo story” guidance remains in active docs.

## References

- [FR-021](../../../fr-br/FR-021-fr-br-uxr-repository-stories.md)
- [FR-072](../../../fr-br/FR-072-uniform-repository-abstract-space-kanban-rules.md)
