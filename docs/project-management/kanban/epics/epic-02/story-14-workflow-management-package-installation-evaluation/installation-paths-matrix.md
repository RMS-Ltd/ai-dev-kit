...
| OPT-INSTALL-ERRORS | Optional | Install error code surface (FR-108) | `adk_install_errors.py`; `config/install-error-codes.yaml` | Y | Y | N/A | N | N | `ADK-I*` codes in console | `packages/frameworks/workflow-mgt/config/install-error-codes.yaml`; `docs/documentation/user-docs/framework-dependency-troubleshooting-guide.md` | Undocumented (S13) | TBD | `generate_install_error_docs.py` is maintainer-only |
