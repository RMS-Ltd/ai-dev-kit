...
Deliver [FR-118](../../../fr-br/FR-118-promote-kanban-to-docs-kanban-retire-project-management-shell.md): inventory-first promotion of the book-repo Kanban tree from `docs/project-management/kanban/` to **`docs/kanban/`**, deletion of vestigial `docs/project-management/rituals/`, and repository-wide path rewire before RC.
...
- **IPW planning artifact:** `IPP-E07S01T12-*.md` (to be filed under `docs/implementation-cycles/` before implementation)
- **Phase 0 inventory:** `docs/governance/KANBAN-REHOUSING-INVENTORY.md` (or `docs/maintenance/` — path decided in IPW Wave 0)
...
- [ ] **AC0:** Kanban rehousing inventory complete and **maintainer-approved** before any `git mv`.
- [ ] **AC1:** [FR-118](../../../fr-br/FR-118-promote-kanban-to-docs-kanban-retire-project-management-shell.md) FR-118-F1..F8 satisfied.
- [ ] **AC2:** IPP linked from this task before implementation (`IPP-E07S01T12-*.md`).
- [ ] **AC3:** `kanban_root: docs/kanban` in `rw-config.yaml`; RW Step 7 four-surface reconciliation passes on final wave.
- [ ] **AC4:** Active-surface `rg` for `project-management/kanban` returns clean (changelog archive exempt per FR-118-F7).
- [ ] **AC5:** CI parity green on `dev` after final wave.
...
- [ ] Wave 0: Inventory drafted
- [ ] Wave 0: Maintainer sign-off
- [ ] Wave 1: IPP filed and linked
- [ ] Wave A: Rituals removed
- [ ] Wave B: `docs/kanban/` live
- [ ] Waves C–D: Config + framework rewired
- [ ] Wave E: Verification green
- [ ] Wave F: Adopter migration docs
- [ ] FR-118 closure + task COMPLETE
