...
- [ ] Validators resolve `packages/frameworks/workflow-mgt` in ai-dev-kit monorepo.
- [ ] Legacy `packages/frameworks/workflow mgt` still supported when present.
- [ ] **`run_workflow_scripts_ci_pytest.sh`** — 105 tests pass (0 failures).
- [ ] **`.github/workflows/workflow-scripts-pytest.yml`** green on `dev`/`main`.
