# Changelog v0.4.19.5+2

**Release Date:** 2026-04-25 17:21:53 UTC  
**Internal Version:** 0.4.19.5+2  
**Epic / Story / Task:** E4 | S19 | T05

## Summary

`RW E4:S19:T05 --art` publishes the planning package for filename normalization and establishes implementation-readiness artifacts for the rehoused UXR-008 scope.

## Change implemented

- Incremented internal version to `v0.4.19.5+2`
- Updated SemVer-facing metadata to `v0.4.764+2`
- Added IPP planning artifact for `E4:S19:T05`
- Added ICW specification, test-design, and implementation-plan artifacts for `E4:S19:T05`
- Cross-linked T05 task references to the new planning package

## References

- **Task:** [T05-kanban-and-fr-br-uxr-filename-normalization-uxr008.md](docs/project-management/kanban/epics/epic-04/story-19-fr-br-uxr-abstract-governance-and-intake/T05-kanban-and-fr-br-uxr-filename-normalization-uxr008.md)
- **IPP:** [IPP-E4S19T05-kanban-fr-br-uxr-filename-normalization.md](docs/implementation-cycles/IPP-E4S19T05-kanban-fr-br-uxr-filename-normalization.md)
- **ICW:** [ICW-E4S19T05-specification.md](docs/implementation-cycles/ICW-E4S19T05-specification.md)
