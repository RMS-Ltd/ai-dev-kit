...
- Findings:
  - FR-084 violation: four-surface report omits expected surface(s): ['fbu_doc'].
  - FR-084 violation: untouched surface(s) lack rationale notes (must explain why no mutation): ['task_doc', 'kboard'].
- Evidence:
  - `report`: `/Users/ruari/Projects/ai-dev-kit/docs/changelog-and-release-notes/changelog-archive/four-surface-reports/rw-step7-four-surface-report-v0-8-3-22plus2-e8s3t22.json`
  - `task_doc`: `{'touched': False, 'paths': [], 'notes_count': 0}`
  - `kboard`: `{'touched': False, 'paths': [], 'notes_count': 0}`
  - `release_scope`: `{}`
